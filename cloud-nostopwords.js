...
function generate() {
  let scale = (fontSize = d3.scale[d3.select("input[name=scale]:checked").property("value")]().range([10, 100]))
  //console.log(scale)
...
function parseText(t) {
  ...
  t.split(n ? /\n/g : wordSeparators).forEach(function (t) {
    ...
    //console.log(tags)
    //let freqList = Object.keys(tags).map(tag => tags[tag].value)
    //console.log(freqList)
    
    //console.log(standardDeviation(freqList))
  
    generate();
}
function generate() {
  let scaleValue = d3.select("input[name=scale]:checked").property("value")
  //console.log(scaleValue)
  let scale = (fontSize = d3.scale[scaleValue]().range([10, 100]))
  //console.log(scale)
...
d3.select("#download-svg").on("click", downloadSVG), d3.select("#download-png").on("click", downloadPNG);
var form = d3.select("#form").on("submit", function () {
  //console.log(parseText(d3.select("#text").property("value")))
